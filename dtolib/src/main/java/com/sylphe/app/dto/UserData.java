...
public class UserData {
    ...
    public UserData(int id, int userProperties, LocData location){
        this.id = id;
    ...
    public UserData(byte[] userStream) {
        ...
        userProperties = ByteBuffer.wrap(userStream,4,4).order(ByteOrder.LITTLE_ENDIAN).getInt();
        byte[] bytes = new byte[16];
        ByteBuffer.wrap(userStream,8,16).get(bytes,0,bytes.length);
        locData = new LocData(bytes);
    }

    public UserData(int userId, int userProperties) {
        this.id = userId;
        this.userProperties = userProperties;
    ...
    public byte[] getStream(){
        ...
        ToByteUtil.intToBytes_LE(userProperties,stream,4);
